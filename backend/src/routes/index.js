...
const telefonesEmpresa = createGenericService({
  table: "telefones_empresa",
  pk: "id",
  columns: ["empresa_id", "telefone"],
});
const telefonesCliente = createGenericService({
  table: "telefones_cliente",
  pk: "id",
  columns: ["cliente_id", "telefone"],
});
const guindastes = createGenericService({
  table: "guindastes",
  pk: "servico_id",
  columns: ["servico_id", "tamanho_base", "altura", "bonus"],
});
const transportes = createGenericService({
  table: "transportes",
  pk: "servico_id",
  columns: ["servico_id", "limite_carga", "percentual_acrescimo"],
});
const oferecem = createGenericService({
  table: "oferecem",
  pk: "id",
  columns: ["empresa_id", "servico_id"],
});
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    COUNT(DISTINCT p.codigo) AS total_servicos,
    COALESCE(SUM(ip.preco), 0) AS valor_total
    FROM cidades c
    LEFT JOIN pedidos p ON p.cidade_partida = c.id
    LEFT JOIN itens_pedido ip ON ip.pedido_id = p.codigo
    GROUP BY c.nome
    ORDER BY total_servicos DESC, c.nome ASC
  `);
    res.json(
      rows.map((r) => ({
        cidade: r.cidade,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    COUNT(DISTINCT p.codigo) AS total_servicos,
    COALESCE(SUM(CASE WHEN ip.data_fim IS NOT NULL THEN ip.preco ELSE 0 END), 0) AS valor_ganho
    FROM empresas e
    LEFT JOIN pedidos p ON p.empresa_id = e.id
    LEFT JOIN itens_pedido ip ON ip.pedido_id = p.codigo
    GROUP BY e.nome
    ORDER BY total_servicos DESC, e.nome ASC
  `);
    res.json(
      rows.map((r) => ({
        empresa: r.empresa,
        total_servicos: Number(r.total_servicos),
        valor_ganho: Number(r.valor_ganho),
      })),
    );
  }),
);
