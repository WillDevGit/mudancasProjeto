...
type Empresa = {
  ...
  nome: string;
  endereco: string;
  telefones_empresa: { id: number; telefone: string | null }[];
};
...
const schema = z.object({
  ...
  telefones: z.array(
    z.object({ telefone: z.string().trim().min(1, "Telefone obrigatório").max(30) }),
  ),
});
...
function EmpresasPage() {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: { nome: "", endereco: "", telefones: [] },
  });
  const phones = useFieldArray({ control: form.control, name: "telefones" });

  const openNew = () => {
    setEditing(null);
    form.reset({ nome: "", endereco: "", telefones: [{ telefone: "" }] });
    setDialogOpen(true);
  };
  const openEdit = (e: Empresa) => {
    ...
    form.reset({
      nome: e.nome,
      endereco: e.endereco,
      telefones: e.telefones_empresa.map((t) => ({ telefone: t.telefone ?? "" })),
    });
    setDialogOpen(true);
  };
  ...
  const filtered = useMemo(() => {
    ...
    const filt = q
      ? data.filter((e) => e.nome.toLowerCase().includes(q) || e.endereco.toLowerCase().includes(q))
      : data;
  ...
  return (
    <div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    onClick={() => phones.append({ telefone: "" })}
                  >
                    <Plus className="h-3 w-3" /> Adicionar
                  </Button>
                </div>
                {phones.fields.length === 0 && (
                  <p className="text-xs text-muted-foreground">Nenhum telefone adicionado.</p>
                )}
                {phones.fields.map((f, i) => (
                  <div key={f.id} className="flex gap-2">
                    <FormField
                      control={form.control}
                      name={`telefones.${i}.telefone`}
                      render={({ field }) => (
      ...
      <ConfirmDelete
        ...
        description={`Excluir a empresa "${deleting?.nome}"?`}
      />
    </div>
  );
}
